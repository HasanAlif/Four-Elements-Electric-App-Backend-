...
import { OUTLET_INSTALL_TYPES } from './Outlets.interface';

const outletsBodySchema = z.object({
  ...
  intendedUseOfOutlets: z.string().optional(),
  howManyOutletsNeeds: z.string().optional(),
  newInstallationOrReplacement: z.enum(OUTLET_INSTALL_TYPES).optional(),
  photosOfWhereOutletsInstall: z.array(z.string()).optional(),
  typeOfOutletsNeed: z.string().optional(),
  photosOfCurrentOutlets: z.array(z.string()).optional(),
  howManyAmps: z.string().optional(),
  ampsOrVoltsNeeded: z.string().optional(),
...
export const OutletsValidation = {
      ...
      .superRefine((data, ctx) => {
        const res =
          data.status === Service_STATUSES.DRAFT
            ? outletsBodySchema.partial().safeParse(data)
            : outletsBodySchema.safeParse(data);
        if (!res.success) {
  ...
  idParamsSchema: z.object({
    params: z.object({
      id: z.string({ error: 'Outlet request ID is required!' }).min(1),
    }),
  }),
