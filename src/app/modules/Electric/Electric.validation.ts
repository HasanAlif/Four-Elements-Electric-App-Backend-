...
const electricBodySchema = z.object({
...
export const ElectricValidation = {
      ...
      .superRefine((data, ctx) => {
        const res =
          data.status === Service_STATUSES.DRAFT
            ? electricBodySchema.partial().safeParse(data)
            : electricBodySchema.safeParse(data);
        if (!res.success) {
  ...
  idParamsSchema: z.object({
    params: z.object({
      id: z.string({ error: 'Electric request ID is required!' }).min(1),
    }),
  }),
