...
const newConstructionBodySchema = z.object({
  ...
  hasConstructionBegun: z.boolean().optional(),
  stageOfConstruction: z.string().optional(),
  haveBuildingPlans: z.boolean().optional(),
  photosOfBuildingPlans: z.array(z.string()).optional(),

  status: z.enum(Service_STATUSES).optional(),
  completionPercentage: z.number().optional(),
});

export const NewConstructionValidation = {
      ...
      .superRefine((data, ctx) => {
        const res =
          data.status === Service_STATUSES.DRAFT
            ? newConstructionBodySchema.partial().safeParse(data)
            : newConstructionBodySchema.safeParse(data);
        if (!res.success) {
  ...
  idParamsSchema: z.object({
    params: z.object({
      id: z
        .string({ error: 'New construction request ID is required!' })
        .min(1),
    }),
  }),
  ...
  updateSchema: z.object({
    ...
    body: newConstructionBodySchema
      .partial()
