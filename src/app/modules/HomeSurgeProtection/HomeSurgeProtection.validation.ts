...
const homeSurgeProtectionBodySchema = z.object({
  ...
  photosOfElectricalPanel: z.array(z.string()).optional(),
...
export const HomeSurgeProtectionValidation = {
      ...
      .superRefine((data, ctx) => {
        const res =
          data.status === Service_STATUSES.DRAFT
            ? homeSurgeProtectionBodySchema.partial().safeParse(data)
            : homeSurgeProtectionBodySchema.safeParse(data);
        if (!res.success) {
  ...
  idParamsSchema: z.object({
    params: z.object({
      id: z
        .string({ error: 'Home surge protection request ID is required!' })
        .min(1),
    }),
  }),
  ...
  updateSchema: z.object({
    ...
    body: homeSurgeProtectionBodySchema
      .partial()
